...
async function getCatwomanReply(channelId, authorId, authorName, userMessage) {
  ...
  try {
    ...
    for (const [emojiName, emojiCode] of Object.entries(emojiDictionary)) {
      ...
      const regexBroken = new RegExp(`:${emojiName}(?!\\w)`, 'gi'); // صيد الإيموجي المقطوع الذي لا ينتهي بنقطتين
      
      reply = reply.replace(regexBracket, emojiCode)
                   .replace(regexStandard, emojiCode)
                   .replace(regexBroken, emojiCode);
    }

    // تنظيف النصوص والرموز والنقاط العشوائية المقلوبة في البداية والنهاية
    reply = reply.replace(/\[الشخص:?\s*[^\]]*\]/g, '').trim();
    reply = reply.replace(/<@!?\d+>/g, '').replace(/@\w+/g, '').trim();
    
    // إزالة علامات الترقيم المشوهة الناتجة عن تداخل اللغات في ديسكورد
...
client.once('ready', () => { console.log('Catwoman Final Clean Fix Active! 🐾🔒'); });
