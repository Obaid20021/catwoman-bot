...
// ===== مصفوفة الرسائل التلقائية غياب محمد (3 مرات يومياً) =====
...
async function registerSlashCommands() {
  ...
  try {
    ...
    console.error('❌ فشل تسجيل Slash Commands:', err);
  }
}
...
// ===== جاهز ونظام الرسائل التلقائية (3 مرات يومياً) =====
  ...
  setInterval(async () => {
    ...
    try {
      ...
      if (channel && channel.isTextBased()) {
        ...
        }, 2000); // محاكاة الكتابة لمدة ثانيتين
      }
    } catch (err) {
      console.error('🚨 فشل إرسال الرسالة التلقائية لكاتوومان:', err);
    }
  }, 8 * 60 * 60 * 1000); // 8 ساعات بالملي ثانية
});
