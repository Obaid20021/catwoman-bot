...
function resolveIdentity(userId) {
  ...
  if (known) {
    return `${known.name} — من الأعضاء المعروفين لديها، طبيعة علاقتهما: ${known.relation}`;
  }
...
async function generateResponse(userId, userName, messageText) {
  try {
    ...
    // إزالة أي بادئة بين قوسين مثل [كات]: أو [المتحدث]: قد يضيفها النموذج بالخطأ
    reply = reply.replace(/^\[.*?\]\s*:?\s*/g, '').trim();
    reply = reply.replace(/^الرسالة:\s*/gim, '').trim();
